...
class ScheduleListController: UIViewController {
 ...
 public var dataPersistence: DataPersistence <Event>!
  ...
  @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
    ...
    // we use this function to connect this VC to the CreateEventController (usually we do it in strotyboard, but now we will use code for that)
   // Before that we have to give ID to CV (inside Identity Inspector, give that same name as class name ex. “CreateEventController”
    private func showCreateEventVC(_ event: Event? = nil) { // Event? = nil , this is giving a defaul value of nil, basically we/user may add parameter or may leave empty
        ...
        // let createNewVC = CreateEventController() - creates an empty view controller without any outlets, etc. will crash if you are expecting your outlets to work if they exist in the storyboard
      
        createEventController.event = event

        // for updating an event we will inject ("dependancy enjection") the selected event
        // createEventController.event = event
...
extension ScheduleListController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let event = events[indexPath.row]
        showCreateEventVC(event) // this send us to another VC
    }
