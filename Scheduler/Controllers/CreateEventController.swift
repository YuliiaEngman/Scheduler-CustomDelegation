...
class CreateEventController: UIViewController {
  ...
  // private for setting (it is public for reading outside of VC)
  // public for getting (but changing just inside here)
  ...
  private func updateUI() {
    if let event = event { // coming from didSelectRowAt (if the event already excist)
      self.event = event
